...
def read_and_decode(tfrecords_filename, im_res, vertex_count, batch_size, dynamic_range, augment_dataset=True):
    ...
    features = tf.parse_single_example(
        ...
        features={
            ...
            'raster_polygon_raw': tf.FixedLenFeature([], tf.string)
        })
    ...
    raster_polygon_flat = tf.decode_raw(features['raster_polygon_raw'], tf.uint8)

    # Reshape tensors
    image_shape = tf.stack([height, width, 3])
    polygon_shape = tf.stack([vertex_count, 2])
    raster_polygon_shape = tf.stack([height, width, 1])
    image = tf.reshape(image_flat, image_shape)
    polygon = tf.reshape(polygon_flat, polygon_shape)
    raster_polygon = tf.reshape(raster_polygon_flat, raster_polygon_shape)
    ...
    if augment_dataset:
        ...
        raster_polygon = tf.contrib.image.rotate(raster_polygon, angle, interpolation='BILINEAR')
    # Apply crop to image
    if augment_dataset:
    ...
    raster_polygon = tf.image.resize_image_with_crop_or_pad(image=raster_polygon,
                                                   target_height=crop_res,
                                                   target_width=crop_res)
    ...
    if augment_dataset:
        ...
        image, polygon, raster_polygon = tf.cond(0.5 <= flip,
                                 lambda: (
                                 tf.image.flip_up_down(image), polygon_flip_up_down(polygon, vertex_count, crop_res), tf.image.flip_up_down(raster_polygon)),
                                 lambda: (image, polygon, raster_polygon))
        ...
        # raster_polygon = tf.image.crop_to_bounding_box(
        #     raster_polygon,
        #     crop_offset[1],
        #     crop_offset[0],
        #     im_res,
    ...
    raster_polygon = raster_polygon / 255

    # Binaries raster_polygon
    raster_polygon = 0.5 < raster_polygon

    min_queue_examples = 256
    images, polygons, raster_polygons = tf.train.shuffle_batch([image, polygon, raster_polygon],
                                                               ...
                                                               seed=SEED)
    return images, polygons, raster_polygons


if __name__ == "__main__":
    # --- Params --- #
    tfrecord_filepath = "/home/nigirard/epitome-polygon-deep-learning/data/photovoltaic_array_location_dataset/tfrecords.unet_and_vectorization/" \
                        "val.tfrecord"
    im_res = 67
    vertex_count = 4
    batch_size = 256
    ...
    # queue.
    image, polygon, raster_polygon = read_and_decode(tfrecord_filepath, im_res, vertex_count, batch_size, dynamic_range,
    ...
    with tf.Session() as sess:
        ...
        for i in range(1):
            imgs, polys, raster_polys = sess.run([image, polygon, raster_polygon])
