...
def loss_and_accuracy(y_, y, batch_size, correct_dist_threshold):
    ...
    with tf.name_scope('accuracy'):
        ...
        # Second accuracy, with 2x correct_dist_threshold
        correct_prediction2 = tf.less(aligned_poly_l2diffs, 2 * correct_dist_threshold)
        ...
        # Second accuracy, with 3x correct_dist_threshold
        correct_prediction3 = tf.less(aligned_poly_l2diffs, 3 * correct_dist_threshold)
        correct_prediction3 = tf.cast(correct_prediction3, tf.float32)
        accuracy3 = tf.reduce_mean(correct_prediction3)
        tf.summary.scalar('accuracy3', accuracy3)

    return objective_loss, accuracy, accuracy2, accuracy3
...
def main(_):
    ...
    loss, accuracy, accuracy2, accuracy3 = loss_and_accuracy(y_, y, batch_size, correct_dist_threshold)
